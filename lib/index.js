...
function emitEventCache(target, type, eventObj) {
  ...
  evt.target = target;
  if (eventObj) {
    _Object$assign(evt, toxicPredicateFunctions.isObject(eventObj) ? eventObj : { data: eventObj });
  }
...
function addEventCache(target, type, handler) {
  ...
  if (toxicPredicateFunctions.isFunction(isOnce) && !handlerWrap) {
    handlerWrap = isOnce;
    isOnce = undefined;
  }
  ...
  var handlerWrap = function handlerWrap(e) {
    ...
    if (targetElArr.find) {
      ...
      targetElArr.forEach(function (seEl) {
        return !retEl && targetEls.forEach(function (tgEl) {
          if (!retEl && seEl === tgEl) {
            retEl = tgEl;
          }
        });
